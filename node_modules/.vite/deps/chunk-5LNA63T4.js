...
function startBatchWithReplica(configuration, primary, replica, reportError, pageMayExitObservable, sessionExpireObservable, batchFactoryImp = createBatch) {
...
function startTelemetry(telemetryService, configuration, reportError, pageMayExitObservable, createEncoder) {
...
function startTelemetryCollection(telemetryService, configuration, observable) {
  ...
  function toTelemetryEvent(telemetryService2, rawEvent, runtimeEnvInfo2) {
    const event = {
      type: "telemetry",
      date: timeStampNow(),
  ...
  if (canUseEventBridge()) {
    ...
    const telemetryBatch = startBatchWithReplica(
      ...
      configuration.replica && {
        ...
        encoder: createEncoder(
          5
          /* DeflateEncoderStreamId.TELEMETRY_REPLICA */
        )
      },
      reportError,
      pageMayExitObservable,
...
function startSessionStore(sessionStoreStrategyType, configuration, productKey, computeTrackingType2) {
...
function isUnsupportedExtensionEnvironment(windowLocation, stack = new Error().stack) {
...
var WARN_DOES_NOT_HAVE_ALLOWED_TRACKING_ORIGIN = "Running the Browser SDK in a Web extension content script is discouraged and will be forbidden in a future major release unless the `allowedTrackingOrigins` option is provided.";
var ERROR_NOT_ALLOWED_TRACKING_ORIGIN = "SDK initialized on a non-allowed domain.";
function isAllowedTrackingOrigins(configuration, windowOrigin = typeof location !== "undefined" ? location.origin : "", errorStack) {
...
function buildEndpointParameters({ clientToken, internalAnalyticsSubdomain }, trackType, configurationTags, api, { retry, encoding }) {
  const tags = [`sdk_version:${"6.13.0"}`, `api:${api}`].concat(configurationTags);
  if (retry) {
  ...
  const parameters = [
    ...
    `dd-evp-origin-version=${encodeURIComponent("6.13.0")}`,
...
function startDefaultContext(hooks, configuration, sdkName) {
  hooks.register(0, ({ eventType }) => ({
    ...
    _dd: {
      ...
      browser_sdk_version: canUseEventBridge() ? "6.13.0" : void 0,
      sdk_name: sdkName
    },
    application: {
...
//# sourceMappingURL=chunk-5LNA63T4.js.map
